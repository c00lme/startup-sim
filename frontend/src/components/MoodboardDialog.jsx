...
export default function MoodboardDialog({ moodboard, onClose }) {
  ...
  // Improved regex to handle potential query strings/fragments and be case-insensitive
  // Still focuses on common image extensions
  const urlRegex =
    /https?:\/\/[\w\-.]+\.[a-zA-Z]{2,}(\/\S+\.(?:jpe?g|png|webp|svg|gif))(?:\?[^\s]*)?/gi;
  const urls = Array.from(moodboard.matchAll(urlRegex)).map((m) => m[0]);

  // Extract description by removing URLs and trimming whitespace
          ...
          {urls.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {urls.map((url, i) => (
                <div
                  key={i}
                  className="aspect-square relative overflow-hidden rounded-lg border border-gray-200 shadow-sm"
                >
                  <img
                    src={url}
